...
class AtrousToNativeTest : public ::testing::Test {
 protected:
  template <class TConvOp>
  void TestAtrousToNative() {
    ...
    TF_ASSERT_OK(AtrousToNative(original_graph_def, {{}, {"output"}},
                                   &modified_graph_def));
...
TEST_F(AtrousToNativeTest, TestAtrousToNativeConv2d) {
  TestAtrousToNative<::tensorflow::ops::Conv2D>();
}
TEST_F(AtrousToNativeTest, TestAtrousToDepthwiseConv2dNative) {
  TestAtrousToNative<::tensorflow::ops::DepthwiseConv2dNative>();
}
